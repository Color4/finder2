$(function () {
    ...
    $(".btn-post").click(function () {
        var last_feed = $(".stream li:first-child").attr("feed-id"); // feed-id = pk , 获取主键
        if (last_feed == undefined) {
            last_feed = "0";
        }
        $("#compose-form input[name='last_feed']").val(last_feed); //传递给hidden字段
        $.ajax({
            url: '/feeds/post/',
            data: $("#compose-form").serialize(), // 将form变成标准查询字符串， a=1&b=2&c=3&d=4&e=5
            type: 'post',
            cache: false,
